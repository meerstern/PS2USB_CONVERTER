        ...
        USB_HOST_HID_MOUSE_EVENT event, void * pData)
{
    ...
    switch ( event)
    {
        ...
        case USB_HOST_HID_MOUSE_EVENT_REPORT_RECEIVED:
            ...
            memcpy(&appData.data_m, pData, sizeof(appData.data_m));

            for(loop = 0; loop < USB_HOST_HID_MOUSE_BUTTONS_NUMBER; loop ++)
            {
                if((appData.data_m.buttonState[loop]) &&
                        (USB_HID_USAGE_ID_BUTTON1 == appData.data_m.buttonID[loop]))
                {
                    /* BUTTON1 pressed */
                   //LED1_On();
                }
                if((appData.data_m.buttonState[loop]) &&
                        (USB_HID_USAGE_ID_BUTTON2 == appData.data_m.buttonID[loop]))
                {
                    //LED1_Off();
                    /* BUTTON2 pressed */
...
void APP_USART_Tasks(void)
{
    ...
    switch(appData.usartTaskState)
    {
        ...
        case APP_USART_STATE_DRIVER_WRITE:
            ...
            //UART2_Write(appData.string,sizeof( appData.string )); 
...
/******************************************************************************
  ...
  Remarks:
    See prototype in app.h.
 */
extern int8_t 		Ps2Init;
extern int          Hid_Mode;
extern uint8_t      vinkeylen;
extern uint8_t      vinkeylenold;
uint16_t            cnt;
...
//    bool foundInLast = false;
    ...
    {
        cnt++;
        if(cnt>4000 && Ps2Init==true)
        {
            if(vinkeylen!=0)
            {
                LED_Set();
                SendPS2StrAdditional();
                SendPS2Str();
                LED_Clear();
            }
            cnt=2500;            
